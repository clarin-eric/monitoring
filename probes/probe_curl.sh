...
probe_central_discovery_service_json() {
    ...
    # $3: Optional extra command-line parameter(s) for curl.
    _probe_curl_http_get "${1}" "${2}" 'application/json' $3 &&
    python3 -I -c 'from json import load; from sys import stdin, exit; json_obj=load(stdin); previous_sum=962; current_sum=sum(country["count"] for country in json_obj["countries"]); exit(0 if current_sum >= previous_sum else 2)' <"${temp_data_file_path}" 2>&1 ||
    return 2 ;
}
